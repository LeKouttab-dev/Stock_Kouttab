...
def display_stock_management(user_id, user_role):
    ...
    # --- Vue 2: Affichage des sous-catégories ou des produits ---
        ...
        # Si la catégorie n'a pas de sous-catégories, on affiche directement les produits
        else:
                    ...
                    with st.container(border=True):
                        ...
                        with col3:
                            if user_role in ['AdminBenevoles', 'Super Admin']:
                                ...
                                if st.button("Demander une modification", key=f"request_{row['id']}", use_container_width=True):
                                    request_stock_change_dialog(user_id, row['id'], row['nom'], row['quantite'])
    ...
    # --- Vue 1: Affichage des catégories principales ---
        ...
        grid_cols = [col1, col2] * ((len(category_list) + 1) // 2)
        ...
        for i, category in enumerate(category_list):
            with grid_cols[i]:
                with st.container(border=True):
                    ...
                    if st.button(f"Consulter {category}", key=f"cat_{category}", use_container_width=True):
