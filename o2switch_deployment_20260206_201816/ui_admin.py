...
def display_admin_page(user_id, user_role):
    ...
    if user_role in ['AdminBenevoles', 'Super Admin']:
        ...
        all_categories = get_all_categories()  # Récupérer une seule fois
        ...
        if not items_to_manage.empty:
            for index, row in items_to_manage.iterrows():
                with st.expander(f"{row.get('emoji', '📦')} Modifier/Supprimer : **{row['nom']}**"):
                    ...
                    # Catégorie dynamique depuis la base de données (réactive)
                    edit_categorie = st.selectbox("Catégorie", options=all_categories, 
                                    index=all_categories.index(row['categorie']) if pd.notna(row['categorie']) and row['categorie'] in all_categories else 0, 
                                    key=f"edit_cat_{row['id']}")
                    
                    # Sous-catégorie dynamique selon la catégorie sélectionnée (réactive)
                    edit_sous_categorie = None
                    subcat_options = CATEGORIES.get(edit_categorie, [])
                    
                    if subcat_options:  # S'il y a des sous-catégories pour cette catégorie
                        current_sc_index = subcat_options.index(row['sous_categorie']) if pd.notna(row['sous_categorie']) and row['sous_categorie'] in subcat_options else 0
                        edit_sous_categorie = st.selectbox(f"Sous-catégorie pour {edit_categorie}", options=subcat_options, index=current_sc_index, key=f"edit_sc_{row['id']}")
...
def display_manual_item_form(CATEGORIES):
    ...
    # Sélection de catégorie en dehors du formulaire pour la réactivité
    all_categories = get_all_categories()
    selected_category = st.selectbox("Catégorie", options=all_categories, key="new_item_category")
    
    # Sous-catégorie dynamique selon la catégorie sélectionnée
    subcat_options = CATEGORIES.get(selected_category, [])
    selected_subcategory = None
    if subcat_options:  # S'il y a des sous-catégories pour cette catégorie
        selected_subcategory = st.selectbox(f"Sous-catégorie pour {selected_category}", options=subcat_options, key="new_item_subcategory")
    ...
    with st.form("add_item_form"):
        ...
        if add_submitted:
            if new_nom and new_emoji:
                if add_item(new_nom, selected_category, selected_subcategory, new_quantite, new_seuil_alerte, new_emoji):
