...
def init_db():
    ...
    for category in default_categories:
        c.execute("INSERT OR IGNORE INTO Categories (nom, is_default) VALUES (?, 1)", (category,))
...
def get_all_categories():
    """Récupère toutes les catégories de la table Categories"""
    conn = sqlite3.connect(DATABASE_PATH)
    c = conn.cursor()
...
def update_category(old_name, new_name):
    """Met à jour une catégorie dans la table Categories et dans les articles"""
    conn = sqlite3.connect(DATABASE_PATH)
    c = conn.cursor()
    try:
        # Vérifier si c'est une catégorie par défaut
        c.execute("SELECT is_default FROM Categories WHERE nom = ?", (old_name,))
        result = c.fetchone()
        if result and result[0] == 1:
            return False, "Impossible de modifier une catégorie par défaut"
        
        # Vérifier si le nouveau nom existe déjà
        c.execute("SELECT COUNT(*) FROM Categories WHERE nom = ?", (new_name,))
        if c.fetchone()[0] > 0:
            return False, "Ce nom de catégorie existe déjà"
        
        # Mettre à jour la catégorie dans la table Categories
        c.execute("UPDATE Categories SET nom = ? WHERE nom = ?", (new_name, old_name))
...
def delete_category(category_name):
    """Supprime une catégorie de la table Categories"""
    conn = sqlite3.connect(DATABASE_PATH)
    c = conn.cursor()
    try:
        # Vérifier si c'est une catégorie par défaut
        c.execute("SELECT is_default FROM Categories WHERE nom = ?", (category_name,))
        result = c.fetchone()
        if result and result[0] == 1:
            return False, "Impossible de supprimer une catégorie par défaut"
        ...
        c.execute("DELETE FROM Categories WHERE nom = ?", (category_name,))
...
def add_category(category_name):
    ...
    try:
        ...
        c.execute("INSERT INTO Categories (nom, is_default) VALUES (?, 0)", (category_name,))
        conn.commit()
        logger.info(f"Nouvelle catégorie '{category_name}' ajoutée")
        return True, "Catégorie ajoutée avec succès"
    except sqlite3.IntegrityError:
        logger.warning(f"Tentative d'ajout d'une catégorie existante : '{category_name}'")
...
def add_subcategory(category_name, subcategory_name):
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM SousCategories WHERE nom_categorie = ? AND nom_sous_categorie = ?", 
        ...
        c.execute("INSERT INTO SousCategories (nom_categorie, nom_sous_categorie) VALUES (?, ?)", 
                 (category_name, subcategory_name))
        conn.commit()
        ...
        logger.error(f"Erreur lors de l'ajout de la sous-catégorie: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()
